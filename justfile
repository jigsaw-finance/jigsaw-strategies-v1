...
deploy-jPoints: && _timer
	#!/usr/bin/env bash
	echo "Deploying JigsawPoints on chain " ${CHAIN} "..."
	
	forge script DeployJigsawPoints --rpc-url $CHAIN --slow -vvvv --broadcast --verify --etherscan-api-key ${ETHERSCAN_API_KEY}